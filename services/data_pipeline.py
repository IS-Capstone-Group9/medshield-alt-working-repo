...
# All data outside [VALID_YEAR_MIN, VALID_YEAR_MAX] is rejected.  This enforces
# the canonical analysis window mandated by the MedShield project scope.
VALID_YEAR_MIN: int = 2017
VALID_YEAR_MAX: int = 2025

# ── Area classification ────────────────────────────────────────────────────────
# Only geographic territories receive coordinate-based weather and disease features.
# Customer/channel labels found in the workbook remain valid sales dimensions
# but are intentionally excluded from meteorological joins.
...
# They are tracked for revenue analysis but excluded from weather/disease modelling.
NON_GEOGRAPHIC_LABELS: frozenset[str] = frozenset({
    "Government",
    "Admin",
    "Hospital",
    "Supplies",
...
def _classify_area(area: str | None) -> str:
    """Return the area classification for weather/disease model filtering.

    Returns:
        "geographic"     – a known province/region eligible for weather joins.
        "non_geographic" – a channel or account label (Government, Admin, etc.).
        "unmapped"       – an area value not yet categorised; needs review.
    """
    if not area:
        return "unmapped"
    if area in GEOGRAPHIC_AREAS:
        return "geographic"
    if area in NON_GEOGRAPHIC_LABELS:
        return "non_geographic"
    return "unmapped"


def _extract_source_year(workbook_name: str | None) -> int | None:
    """Derive the intended data year from a workbook filename.

    Expects names like ``medshield_data_2019.csv`` or ``Sales Report 2021.xlsx``.
    Returns None when no 4-digit year can be found.
    """
    if not workbook_name:
        return None
    match = re.search(r"(20\d{2})", str(workbook_name))
    if match:
...
def clean_sales_rows(
    ...
    for source in source_rows:
        ...
        # ── Fix 1.1 — Year range guard ─────────────────────────────────────────
        # Reject any row whose delivery date falls outside the canonical 2017-2025
        # analysis window.  Rows with no delivery date are already rejected above.
        delivery_year = delivery_date.year if delivery_date else None
        if delivery_year is not None and not (VALID_YEAR_MIN <= delivery_year <= VALID_YEAR_MAX):
            notes.append(
                f"delivery year {delivery_year} is outside the analysis range "
                f"{VALID_YEAR_MIN}\u2013{VALID_YEAR_MAX}"
            )
            issue_counts["out_of_range_year"] += 1

        # ── Fix 1.6 — data_source_year and carry-over detection ───────────────
        data_source_year = _extract_source_year(source.workbook)
        if delivery_year is not None and data_source_year is not None and delivery_year != data_source_year:
            notes.append(
                f"delivery year {delivery_year} differs from source workbook year {data_source_year}"
            )
            issue_counts["source_workbook_year_mismatch"] += 1

        # Legacy sheet-year cross-check (kept for xlsx workbooks with year-named tabs)
        source_year = int(source.sheet) if source.sheet.isdigit() and len(source.sheet) == 4 else None
        if delivery_date and source_year and delivery_date.year != source_year and data_source_year is None:
            notes.append(f"delivery year {delivery_date.year} differs from sheet {source_year}")

        # ── Numeric blank-to-zero and sign checks ─────────────────────────────
        for field in (
            "quantity",
            "unit_cost",
            "total_cost",
            ...
            elif field not in {"discount", "net_income", "net_cost"} and row[field] < 0:
        ...
        if float(row["quantity"] or 0) < 0:
            ...
            issue_counts["negative_quantity"] += 1

        # ── Fix 1.2 — Zero quantity / zero trade price warnings ───────────────
        qty_val = float(row["quantity"] or 0)
        ttp_val = float(row["total_trade_price"] or 0)
        if qty_val == 0.0 and ttp_val == 0.0:
            notes.append("zero quantity and zero total trade price")
            issue_counts["zero_quantity_and_revenue"] += 1
        elif ttp_val == 0.0 and qty_val > 0:
            notes.append("zero total trade price with nonzero quantity")
            issue_counts["zero_revenue_nonzero_qty"] += 1
        elif qty_val == 0.0 and ttp_val > 0:
            notes.append("zero quantity with nonzero total trade price")
            issue_counts["zero_qty_nonzero_revenue"] += 1

        # ── Fix 1.4 — Margin anomaly (tightened threshold) ────────────────────
        ...
        # Range check applies to both source-provided and derived values.
        if row["margin_pct"] is not None and (row["margin_pct"] < -0.5 or row["margin_pct"] > 1.5):
            notes.append("margin percentage is outside the expected range (-0.5 to 1.5)")
            issue_counts["margin_anomalies"] += 1

        # ── Multi-Year Statistical Outlier & Data Entry Anomaly Check ─────────
        ...
        # ── Rejection decision ─────────────────────────────────────────────────
        # A row is rejected if any required field is missing OR the delivery year
        # falls outside the canonical 2017-2025 analysis window.
        year_out_of_range = (
            delivery_year is not None
            and not (VALID_YEAR_MIN <= delivery_year <= VALID_YEAR_MAX)
        )
        rejected = not row["area"] or not delivery_date or not row["product"] or year_out_of_range
        ...
        # ── Fix 1.5 — area_type column ────────────────────────────────────────
        area_type = _classify_area(row["area"])

        # ── Fix 1.7 — in_analysis_range boolean ──────────────────────────────
        in_analysis_range = (
            delivery_year is not None
            and VALID_YEAR_MIN <= delivery_year <= VALID_YEAR_MAX
        )

        source_hash = _source_hash(source)
        row.update({
            "year": delivery_year,
            "data_source_year": data_source_year,
            "in_analysis_range": in_analysis_range,
            "area_type": area_type,
