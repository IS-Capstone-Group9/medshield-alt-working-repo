"""
Script to safely escape HTML quotes inside MEDSHIELD_MARKUP string in medshieldReference.ts
"""
from pathlib import Path

target_file = Path("frontend/lib/medshieldReference.ts")

# First run redesign_medshield_ui.py to reset medshieldReference.ts cleanly
import sys
from pathlib import Path
...
# Action Cards + Drill-down table HTML (properly escaped for JS double-quoted string literal)
new_seasonal_section = r'''
...
# Read fresh reset content
content = target_file.read_text(encoding="utf-8")

# CSS rules cleanly escaped inside MEDSHIELD_STYLE
css_clean_rules = (
...
// Initial auto-load for Monsoon on ready
setTimeout(() => {
  selectSeasonRestock('monsoon', document.querySelector('.active-season'));
}, 500);
'''

# Insert CSS into MEDSHIELD_STYLE
if "/* ── DATA UPLOAD ── */" in content:
    content = content.replace("/* ── DATA UPLOAD ── */", css_clean_rules + "/* ── DATA UPLOAD ── */")

# Insert HTML into MEDSHIELD_MARKUP
start_banner_idx = content.find(r"\u003c!-- SEASONAL EPIDEMIC RESTOCK GRID (6 CARDS) --\u003e")
if start_banner_idx == -1:
    start_banner_idx = content.find("<!-- SEASONAL EPIDEMIC RESTOCK GRID (6 CARDS) -->")
end_banner_idx = content.find(r"\u003c!-- PRESCRIPTIVE ALERTS HERO --\u003e")
if end_banner_idx == -1:
    end_banner_idx = content.find("<!-- PRESCRIPTIVE ALERTS HERO -->")

if start_banner_idx != -1 and end_banner_idx != -1:
    content = content[:start_banner_idx] + new_seasonal_section + "\n      " + content[end_banner_idx:]

if "selectSeasonRestock" not in content:
    content += "\n\n" + js_function

target_file.write_text(content, encoding="utf-8")
print("medshieldReference.ts escaped and updated cleanly!")
