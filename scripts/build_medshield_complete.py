"""
Script to cleanly inject Action Card drilldown and selectSeasonRestock JS inside MEDSHIELD_SCRIPT string literal.
"""
...
# First run original redesign_medshield_ui.py to reset cleanly
import sys
...
# 1. Action cards + drilldown table markup
seasonal_markup = r'''
...
# Replace seasonal markup block inside MEDSHIELD_MARKUP string
start_idx = content.find(r"\u003c!-- SEASONAL EPIDEMIC RESTOCK GRID --\u003e")
end_idx = content.find(r"\u003c!-- PRESCRIPTIVE ALERTS HERO --\u003e")

if start_idx != -1 and end_idx != -1:
    content = content[:start_idx] + seasonal_markup + "\n      " + content[end_idx:]

# 2. Append JS function selectSeasonRestock INSIDE MEDSHIELD_SCRIPT double-quoted string
js_code_to_inject = r"""
...
# Format js_code_to_inject safely inside MEDSHIELD_SCRIPT double-quoted string
js_escaped = js_code_to_inject.replace('\n', '\\n').replace('"', '\\"')

# Insert right before ending quote of MEDSHIELD_SCRIPT
if 'export const MEDSHIELD_SCRIPT = "' in content:
    last_quote_idx = content.rfind('"')
    if last_quote_idx != -1:
        content = content[:last_quote_idx] + js_escaped + content[last_quote_idx:]

target_file.write_text(content, encoding="utf-8")
print("Successfully built complete medshieldReference.ts with clean string exports!")
