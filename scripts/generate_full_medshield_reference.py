"""
Script to build full, error-free medshieldReference.ts with interactive Prescriptive Climate-Disease Module
"""
...
.sidebar-brand { padding: 20px; border-bottom: 1px solid #1E293B; }
.brand-logo { display: flex; align-items: center; gap: 12px; min-height: 40px; }
.brand-icon {
  width: 38px; height: 38px; border-radius: 10px;
  display: flex; align-items: center; justify-content: center;
  flex-shrink: 0; overflow: hidden; background: #FFFFFF;
  box-shadow: 0 2px 8px rgba(0,0,0,0.15);
}
.brand-icon img { width: 100%; height: 100%; object-fit: contain; padding: 4px; }
.brand-name { font-family: var(--font-display); font-size: 16px; color: #F8FAFC; line-height: 1.2; font-weight: 700; letter-spacing: -0.01em; }
.brand-sub { font-size: 10px; color: #94A3B8; letter-spacing: 0.08em; text-transform: uppercase; margin-top: 2px; font-weight: 600; }
.brand-text, .nav-label, .nav-section { transition: opacity var(--transition), transform var(--transition); }
...
.nav-section { font-size: 10px; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; color: #64748B; padding: 12px 12px 6px; }
.nav-item {
  display: flex; align-items: center; gap: 12px; min-height: 42px; padding: 10px 14px;
  border-radius: 10px; color: #94A3B8; font-size: 13px; font-weight: 500; cursor: pointer;
  position: relative; transition: all var(--transition); margin-bottom: 4px; white-space: nowrap;
}
.nav-item:hover { background: rgba(255,255,255,0.06); color: #F8FAFC; }
.nav-item.active { background: #0EA5E9; color: #FFFFFF; font-weight: 600; box-shadow: 0 4px 12px rgba(14,165,233,0.35); }
...
.sidebar-avatar { width: 36px; height: 36px; border-radius: 50%; display: inline-flex; align-items: center; justify-content: center; background: #1E293B; color: #38BDF8; font-size: 12px; font-weight: 700; flex-shrink: 0; }
...
.sidebar-footer-btn { width: 36px; height: 36px; border-radius: 8px; display: inline-flex; align-items: center; justify-content: center; background: #1E293B; border: 1px solid #334155; color: #94A3B8; transition: all var(--transition); }
...
.main { margin-left: var(--sidebar-w); flex: 1; display: flex; flex-direction: column; min-height: 100vh; min-width: 0; transition: margin-left var(--transition); }
.topbar { background: var(--bg-surface); border-bottom: 1px solid var(--border); padding: 0 32px; min-height: 68px; display: flex; align-items: center; justify-content: space-between; position: sticky; top: 0; z-index: 50; box-shadow: var(--shadow-sm); }
...
.nav-toggle { width: 38px; height: 38px; display: inline-flex; align-items: center; justify-content: center; border-radius: var(--radius-sm); color: var(--text-secondary); background: var(--bg-elevated); border: 1px solid var(--border); transition: all var(--transition); flex-shrink: 0; }
.nav-toggle:hover { background: var(--border); color: var(--text-primary); }
.topbar-right { display: flex; align-items: center; gap: 12px; }
.topbar-badge { display: flex; align-items: center; gap: 8px; padding: 6px 14px; background: var(--emerald-light); border: 1px solid rgba(16,185,129,0.2); border-radius: 20px; font-size: 11px; color: #047857; font-weight: 600; }
.live-dot { width: 8px; height: 8px; background: #10B981; border-radius: 50%; animation: pulse 2s infinite; }
@keyframes pulse { 0%,100% { opacity: 1; transform: scale(1); } 50% { opacity: 0.4; transform: scale(0.85); } }

.filterbar { display: flex; align-items: center; justify-content: space-between; gap: 16px; width: 100%; max-width: 1600px; margin: 0 auto; padding: 20px 32px 0; }
...
.comp-mode-btn, .yr-btn { padding: 6px 12px; border-radius: 7px; font-size: 12px; font-weight: 600; background: transparent; color: var(--text-secondary); transition: all var(--transition); }
...
.kpi-card { background: var(--bg-surface); border: 1px solid var(--border); border-radius: var(--radius-md); padding: 22px; box-shadow: var(--shadow-sm); transition: all var(--transition); position: relative; overflow: hidden; }
.kpi-card:hover { transform: translateY(-2px); box-shadow: var(--shadow-md); border-color: var(--border-strong); }
.kpi-card::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 4px; background: var(--kpi-accent, var(--accent)); border-radius: var(--radius-md) var(--radius-md) 0 0; }
...
.chart-card { background: var(--bg-surface); border: 1px solid var(--border); border-radius: var(--radius-md); padding: 24px; box-shadow: var(--shadow-sm); transition: all var(--transition); }
...
.hero-panel { background: linear-gradient(135deg, #0F172A 0%, #1E293B 100%); color: #FFFFFF; border-radius: var(--radius-lg); padding: 28px 32px; margin-bottom: 24px; box-shadow: var(--shadow-md); }
...
.seasonal-planner-banner { background: linear-gradient(135deg, #0F172A 0%, #1E293B 100%); border-radius: var(--radius-lg); padding: 26px 30px; margin-bottom: 24px; box-shadow: var(--shadow-md); color: #FFFFFF; }
...
.season-card { background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.12); border-radius: var(--radius-md); padding: 18px 20px; transition: all var(--transition); }
...
.product-table th { text-align: left; padding: 14px 18px; font-size: 11px; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; color: var(--text-muted); background: var(--bg-elevated); border-bottom: 1px solid var(--border); }
...
.abc-pill { display: inline-flex; align-items: center; justify-content: center; width: 24px; height: 24px; border-radius: 50%; font-size: 11px; font-weight: 700; }
...
.alert-banner { background: var(--red-light); border: 1px solid rgba(239,68,68,0.25); border-left: 6px solid var(--red); border-radius: var(--radius-md); padding: 22px 26px; box-shadow: var(--shadow-sm); }
...
.alert-card { background: var(--bg-surface); border: 1px solid var(--border); border-radius: var(--radius-md); padding: 20px; box-shadow: var(--shadow-sm); transition: all var(--transition); border-left: 4px solid var(--border); }
...
.alert-tag { margin-top: 12px; display: inline-flex; align-items: center; padding: 4px 10px; border-radius: 20px; font-size: 10px; font-weight: 700; letter-spacing: 0.06em; text-transform: uppercase; }
...
.section-title { font-family: var(--font-display); font-size: 18px; color: var(--text-primary); font-weight: 700; margin-bottom: 16px; display: flex; align-items: center; gap: 10px; }
...
# Read existing script part
content = target_file.read_text(encoding="utf-8")
script_pos = content.find("export const MEDSHIELD_SCRIPT =")

if script_pos != -1:
    script_part = content[script_pos:]
    # Append selectSeasonRestock JS logic if not present
    if "selectSeasonRestock" not in script_part:
        js_extension = r"""

async function selectSeasonRestock(seasonId, cardEl) {
  try {
    const allCards = document.querySelectorAll('.clickable-season');
    allCards.forEach(c => c.classList.remove('active-season'));
    if (cardEl) {
    ...
    const tableEl = document.getElementById('seasonalDrilldownTable');
    ...
    let data;
    try {
      const res = await fetch('/api/seasonal_restock_detail?season_id=' + seasonId);
      data = await res.json();
    } catch(e) {
    ...
    details.skus.forEach(function(s) {
      let urgencyBadge = '<span class="alert-tag warn">Medium</span>';
...
"""
        script_part = script_part[:-1] + js_extension + '\n"'

    file_out = f'export const MEDSHIELD_STYLE = "{medshield_style.replace(chr(10), "\\n").replace(chr(34), "\\\"")}"\n\nexport const MEDSHIELD_MARKUP = "{medshield_markup.replace(chr(10), "\\n").replace(chr(34), "\\\"")}"\n\n{script_part}'
    target_file.write_text(file_out, encoding="utf-8")
    print("Cleanly generated full medshieldReference.ts with zero syntax errors!")
