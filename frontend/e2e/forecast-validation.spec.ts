...
test('actuals, holdout predictions, empirical bands, metrics and horizon export agree', async ({ page }) => {
  ...
  await expect(page.locator('#forecastWindow')).toContainText('2026-01 to 2026-12')
  ...
  expect(csv).toContain('"2026-06","Future forecast","","2930"')
...
test('scope controls request new evidence; quantity, unknown private and failure states are truthful', async ({ page }) => {
  ...
  expect(await page.evaluate(() => (window as any).Chart.getChart(document.getElementById('forecastChart')).data.datasets[6].data.filter((x: any) => x !== null)[0])).toBe(172)
