...
test('actuals, holdout predictions, empirical bands, metrics and horizon export agree', async ({ page }) => {
  ...
  await expect(page.locator('#forecastWindow')).toContainText('2026-10 to 2027-09')
  await expect(page.locator('#forecastWindow')).toContainText('next 12 months')
  ...
  expect(csv).toContain('"2027-03"')
  expect(csv).toContain('"last_value"')
  expect(csv).not.toContain('2027-04')
...
test('scope controls request new evidence; quantity, unknown private and failure states are truthful', async ({ page }) => {
  ...
  expect(await page.evaluate(() => (window as any).Chart.getChart(document.getElementById('forecastChart')).data.datasets[6].data.filter((x: any) => x !== null)[0])).toBe(181)
