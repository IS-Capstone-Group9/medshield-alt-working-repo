...
export function getExecutableDashboardScript(): string {
  ...
  patchedScript = patchedScript.replace(
    /function renderShowcaseDOMVisuals\(\) \{[\s\S]*?\n\}\s*(?:if \(typeof renderShowcaseDOMVisuals !== 'undefined'\)\s*)?window\.renderShowcaseDOMVisuals = renderShowcaseDOMVisuals;/,
    `function renderShowcaseDOMVisuals() {
  var selected = getActiveDashboardYear();
  var yearRows = Array.isArray(DATA.year_summary) ? DATA.year_summary.slice() : [];
  var years = selected === 'all' || !selected
    ? yearRows.map(function(row) { return String(row.year); }).filter(function(year) { return /^\\d{4}$/.test(year); }).slice(-7)
    : [String(selected)];
  var months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
  var heatData = years.map(function(year) {
    return months.map(function(_, index) {
      var period = year + '-' + String(index + 1).padStart(2, '0');
      var row = (DATA.monthly || []).find(function(item) { return item.period === period; });
      return Number(row && row.revenue) || 0;
    });
  });
  var positive = heatData.flat().filter(function(value) { return value > 0; });
  var maxVal = Math.max.apply(null, positive.length ? positive : [1]);
  var minVal = Math.min.apply(null, positive.length ? positive : [0]);
  var heatContainer = document.getElementById('revenueHeatmapGrid');
  if (heatContainer && years.length) {
    var html = '<div style="display:grid; grid-template-columns: 48px repeat(12, 1fr); gap: 3px; font-size:10px;"><div></div>';
    months.forEach(function(month) { html += '<div style="text-align:center; font-weight:700; color:#67879A; padding:4px 0;">' + month + '</div>'; });
    years.forEach(function(year, yearIndex) {
      html += '<div style="font-weight:700; color:#335F78; display:flex; align-items:center;">' + year + '</div>';
      months.forEach(function(month, monthIndex) {
        var value = heatData[yearIndex][monthIndex];
        var ratio = maxVal > minVal ? (value - minVal) / (maxVal - minVal) : 0;
        var background = ratio > 0.75 ? 'rgba(245, 158, 11, 0.92)' : ratio > 0.5 ? 'rgba(30, 58, 95, 0.85)' : ratio > 0.25 ? 'rgba(51, 95, 120, 0.7)' : 'rgba(201, 219, 229, 0.6)';
        var textColor = ratio > 0.25 ? '#FFFFFF' : '#334155';
        html += '<div class="heatmap-cell" style="background:' + background + '; color:' + textColor + '; height:28px; border-radius:4px;" title="' + year + ' ' + month + ': ₱' + (value / 1000000).toFixed(2) + 'M">' + (value / 1000000).toFixed(2) + '</div>';
      });
    });
    heatContainer.innerHTML = html + '</div>';
  }

  var matrixContainer = document.getElementById('restockMatrixGrid');
  if (matrixContainer) {
    var areaRows = getDashboardTerritoryRows();
    var areaByName = {};
    areaRows.forEach(function(row) { areaByName[row.area] = row; });
    var territories = ['Batangas', 'Quezon', 'Laguna', 'Cavite', 'Bicol'];
    var categories = ['Antipyretics (Non-NSAID)', 'Antibiotics / Leptospirosis', 'IV Hydration Fluids', 'Respiratory / Inhalers', 'GI Rehydration'];
    var categoryWeights = [0.98, 0.92, 0.84, 0.76, 0.88];
    var totalAreaRevenue = areaRows.reduce(function(sum, row) { return sum + (Number(row.revenue) || 0); }, 0) || 1;
    var matrixHtml = '<div style="display:grid; grid-template-columns: 140px repeat(5, 1fr); gap: 4px; font-size:10px;"><div></div>';
    territories.forEach(function(territory) { matrixHtml += '<div style="text-align:center; font-weight:700; color:#335F78; padding:4px 0;">' + territory + '</div>'; });
    categories.forEach(function(category, categoryIndex) {
      matrixHtml += '<div style="font-weight:700; color:#335F78; display:flex; align-items:center; line-height:1.2; font-size:10px;">' + category + '</div>';
      territories.forEach(function(territory, territoryIndex) {
        var area = areaByName[territory] || areaRows[territoryIndex] || { revenue: 0 };
        var share = Math.min(1, Math.max(0, (Number(area.revenue) || 0) / totalAreaRevenue));
        var score = Math.round(Math.min(99, Math.max(45, 45 + share * 45 + categoryWeights[categoryIndex] * 10)));
        var background = score >= 90 ? 'rgba(192, 57, 43, 0.88)' : score >= 80 ? 'rgba(245, 158, 11, 0.88)' : 'rgba(30, 58, 95, 0.8)';
        var badge = score >= 90 ? 'CRITICAL' : score >= 80 ? 'HIGH' : 'MODERATE';
        matrixHtml += '<div class="heatmap-cell" style="background:' + background + '; color:white; height:36px; border-radius:6px; flex-direction:column; padding:2px;" title="' + category + ' → ' + territory + ': Urgency ' + score + '/100"><span style="font-weight:800; font-size:11px;">' + score + '</span><span style="font-size:8px; opacity:0.85;">' + badge + '</span></div>';
      });
    });
    matrixContainer.innerHTML = matrixHtml + '</div>';
  }
}
