...
- Horizon choices are the next 3, 6 or 12 calendar months beginning after the reporting month. The displayed window advances automatically when the Manila calendar month changes; for example, a November 2026 run begins in December 2026. The model origin remains the latest validated closed-month observation and advances only when new validated actuals arrive. Current and future-month source rows are excluded in Manila time. The topbar descriptive year range does not truncate training history. A visible age statement explains when the forecast origin is stale.
